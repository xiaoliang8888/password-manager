...
/**
 * 处理获取所有密码记录的 GET 请求
 * @returns {Promise<NextResponse>} 返回包含所有密码记录的 JSON 响应
 */
export async function GET() {
  try {
    // 使用 Prisma Client 从数据库中查询所有的 PasswordEntry 记录
...
export async function POST(request: Request) {
  try {
    ...
    // 验证数据是否存在，如果缺少任何一项，则返回错误
    if (!website || !username || !password) {
      return NextResponse.json({ error: '网站、用户名和密码不能为空' }, { status: 400 });
    }

    // 使用 Prisma Client 在数据库中创建一条新的 PasswordEntry 记录
